...
class GatePooling(nn.Module):
    ...
    def forward(self, x, need_weights=False, given_weights=None):
        """
        Args:
            x (torch.Tensor): input multimodal embedding (B, n_modal, embed_dim)
            given_weights (torch.Tensor): given gate weights (B, n_comb, n_modal)
        ...
        if given_weights is None:
            ...
            w = alpha.unsqueeze(1).repeat(1, given_weights.shape[1], 1) * given_weights  # (B, n_comb, n_modal)
        x = torch.bmm(w, x)                             # (B, 1, n_dim) /  (B, n_comb, n_dim)
        x = x.squeeze(1)                                # (B, n_dim) / (B, n_comb, n_dim)
