...
class TextEmbeddingExtractor():
    def __init__(self, model_type='bert-large', get_layer_results=False, device='cpu'):
        self.device = device
        self.get_layer_results = get_layer_results
        self.model_type = model_type

        if model_type in ['bert-base', 'bert-large']:
            self.tokenizer = BertTokenizer.from_pretrained(repos_dict[model_type])
            self.model = BertModel.from_pretrained(repos_dict[model_type])
        elif model_type == 'gpt2':
            self.tokenizer = GPT2Tokenizer.from_pretrained(repos_dict[model_type])
            self.model = GPT2Model.from_pretrained(repos_dict[model_type])
        elif model_type in ['t5-base', 't5-large']:
            self.tokenizer = T5Tokenizer.from_pretrained(repos_dict[model_type])
            self.model = T5EncoderModel.from_pretrained(repos_dict[model_type])
        else:
    ...
    """
    def extract(self, txt, save_path=None):
        ...
        segments_ids = [0] * len(indices)
        tokens_tensor = torch.tensor([indices], device=self.device)
        segments_tensor = torch.tensor([segments_ids], device=self.device)
        with torch.no_grad():
            encoded_layers, _ = self.model(tokens_tensor, segments_tensor)
        if self.get_layer_results:
            embed = torch.cat(encoded_layers, dim=0)
        else:
    ...
    def decode(self, x, x_len=None):
        ...
        if self.get_layer_results:
            y = torch.stack(outputs['hidden_states']).transpose(0, 1)
        else:
            y = outputs['hidden_states'][-1].unsqueeze(1)
